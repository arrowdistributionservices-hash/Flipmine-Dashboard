...
export async function fetchDailyPurchasingSummary(debug) {
  ...
  // The weekly-totals block (cols 15-18) is a handful of rows sitting at the top of the
  // sheet, one entry per week — but it only occupies as many rows as there are weeks, which
  // happens to overlap the first several rows of the unrelated *daily* log purely by sheet
  // layout. Each entry's real week has nothing to do with that row's own daily-log date
  // (col 0) — it's simply the Nth week after the log's actual start, 7 days apart.
  const firstLogDate = rows[startIdx] ? parseUsDate(rows[startIdx][0]) : null;
  ...
  for (let i = startIdx; i < rows.length; i++) {
    ...
    if (!inBlock2) {
      ...
      // the "Week" label (col 15) is blank in practice, and even when present shouldn't be
      // trusted over a computed label (see firstLogDate comment above: this block's rows
      // don't correspond 1:1 with the daily log's dates despite sharing row positions).
      const totalWeek = toNum(row[18]);
      if (totalWeek !== null) {
        let weekLabel = row[15];
        if (!weekLabel) {
          if (firstLogDate) {
            const weekDate = new Date(firstLogDate);
            weekDate.setDate(weekDate.getDate() + weekIndex * 7);
            weekLabel = `Week of ${formatUsDate(weekDate)}`;
          } else {
            weekLabel = `Week ${weekIndex + 1}`;
          }
        }
        weekly.push({ week: weekLabel, e2aE2w: toNum(row[16]), wholesale: toNum(row[17]), total: totalWeek });
        weekIndex++;
      }
    } else {
  ...
  if (debug) result._debug = { block2StartIdx, totalRows: rows.length, faqahatCount: Object.keys(perSourceDaily.Faqahat).length, googleCount: Object.keys(perSourceDaily.Google).length, rows0to10: rows.slice(0, 10) };
  return result;
}
