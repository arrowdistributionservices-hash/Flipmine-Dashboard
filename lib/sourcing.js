...
// Column P ("Week") holds a real human-written range like "29th June - 3rd
// July" for the handful of rows in the weekly-totals block — but gviz's
// whole-sheet query silently drops it. That's because column P is reused
// much further down the same sheet for Hasbro's *daily* dates (a second,
// unrelated data block sharing column indices once block 1 ends) — the
// mixed content across the full column confuses gviz's automatic type
// detection, and it nulls out the values it can't reconcile. Scoping the
// query to just the rows that actually hold week-range text sidesteps that
// entirely, since gviz infers the column type from only what's in range.
async function fetchWeekLabels(firstSheetRow, lastSheetRow) {
  const range = `P${firstSheetRow}:P${lastSheetRow}`;
  const url = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent('Daily Purchasing Summary')}&range=${range}`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`status ${res.status}`);
  const text = await res.text();
  const jsonText = text.replace(/^[\s\S]*?setResponse\(/, '').replace(/\);?\s*$/, '');
  const parsed = JSON.parse(jsonText);
  return parsed.table.rows.map(r => r?.c?.[0]?.v ?? null);
}
...
export async function fetchDailyPurchasingSummary(debug) {
  ...
  // Google's CSV export silently drops the title/header rows for the second block
  // ("Faqahat's Sourcing (Arris Items)" / "Google Sourcing") and any blank rows before it —
  // its data rows just continue directly after block 1's last row. The only reliable signal
  // for where block 1 ends and block 2 begins is that the date in column 0 suddenly jumps
  // backward (block 2 restarts its own date sequence from its own start date).
  const perSourceDaily = { Nabeel: {}, Hasan: {}, Faqahat: {}, Google: {}, Mattel: {}, Hasbro: {}, StarWars: {} };
  const weekly = [];
  ...
  let inBlock2 = false;
  let prevDate = null;
  let block2StartIdx = -1;
  let weekIndex = 0;

  // Find the first row where column 0 is an actual date — robust to whether the sheet's
  // title+header occupy one merged row or two separate rows (this has changed before).
  let startIdx = 2;
  for (let i = 0; i < rows.length; i++) {
    if (rows[i] && parseUsDate(rows[i][0])) { startIdx = i; break; }
  }

  for (let i = startIdx; i < rows.length; i++) {
    const row = rows[i];
    if (!row) continue;
    const d = parseUsDate(row[0]);
    if (d) {
      if (!inBlock2 && prevDate && d < prevDate) {
        inBlock2 = true;
        block2StartIdx = i;
      }
      prevDate = d;
    ...
    if (!inBlock2) {
      ...
      // The dollar total (col 18) is the real signal for "this row holds a weekly entry" —
      // the label itself (col 15) comes from fetchWeekLabels() below, since the whole-sheet
      // CSV/JSON query drops it (see that function's comment). "Week N" here is only a
      // placeholder in case that lookup fails; never fabricate a specific date — a wrong
      // guess is worse than an honest "Week N".
      const totalWeek = toNum(row[18]);
      if (totalWeek !== null) {
        weekly.push({ week: `Week ${weekIndex + 1}`, e2aE2w: toNum(row[16]), wholesale: toNum(row[17]), total: totalWeek });
        weekIndex++;
      }
    } else {
      // Second block: Faqahat/Google share the same columns as Nabeel/Hasan did (0-3, 5-8).
      // Mattel/Hasbro/Star Wars were added later, reusing the Totals/Weekly columns (10-13, 15-18)
      // plus a new set (19-22), since those are free once block 1 ends. Each is read independently
      // by its own date column, so row-offset differences between blocks (from their own title/header
      // rows landing at different points) don't cause misalignment.
      addDay(perSourceDaily.Faqahat, row[0], toNum(row[1]), toNum(row[2]));
      addDay(perSourceDaily.Google, row[5], toNum(row[6]), toNum(row[7]));
      addDay(perSourceDaily.Mattel, row[10], toNum(row[11]), toNum(row[12]));
      addDay(perSourceDaily.Hasbro, row[15], toNum(row[16]), toNum(row[17]));
      addDay(perSourceDaily.StarWars, row[19], toNum(row[20]), toNum(row[21]));
    }
  }

  const allDates = new Set();
  Object.values(perSourceDaily).forEach(bucket => Object.keys(bucket).forEach(d => allDates.add(d)));
  const daily = [...allDates].sort((a, b) => parseUsDate(a) - parseUsDate(b)).map(date => {
  ...
  let weekLabelError = null;
  if (weekly.length > 0) {
    try {
      // +2, not +1: the sheet has a two-row header (row 1 = group titles like
      // "Nabeel's LEGO Sourcing", row 2 = field names like "Date"/"Week"),
      // which gviz's CSV export flattens into a single combined header line —
      // so rows[] (0-indexed, one header row) is one short of the true
      // 1-indexed sheet row number (two header rows). Verified directly
      // against the sheet: rows[startIdx] is "6/17/2026" in column A, which
      // is genuinely sheet row 3, not row 2.
      const firstSheetRow = startIdx + 2;
      const lastSheetRow = firstSheetRow + weekly.length - 1;
      const labels = await fetchWeekLabels(firstSheetRow, lastSheetRow);
      labels.forEach((label, i) => {
        if (label && weekly[i]) weekly[i].week = label;
      });
    } catch (e) {
      // Keep the "Week N" placeholders rather than a fabricated date — see
      // the comment on the weekly.push() above for why that matters here.
      weekLabelError = String(e);
    }
  }

  const result = { daily: daily.slice(-14), weekly, bySource: perSourceDaily };
  if (debug) result._debug = { block2StartIdx, totalRows: rows.length, faqahatCount: Object.keys(perSourceDaily.Faqahat).length, googleCount: Object.keys(perSourceDaily.Google).length, rows0to10: rows.slice(0, 10), weekLabelError };
  return result;
}
