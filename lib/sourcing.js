...
export async function fetchDailyPurchasingSummary(debug) {
  ...
  if (debug) result._debug = { block2StartIdx, totalRows: rows.length, faqahatCount: Object.keys(perSourceDaily.Faqahat).length, googleCount: Object.keys(perSourceDaily.Google).length };
  return result;
}
