...
export async function fetchDailyPurchasingSummary(debug) {
  ...
  for (let i = startIdx; i < rows.length; i++) {
    ...
    if (!inBlock2) {
      ...
      // The "Week" label (col 15) sits in a merged cell in the sheet and can come through
      // blank via gviz even when its neighboring dollar amounts are present — so use the
      // dollar total (col 18) as the real signal, not the label text, with a date-based
      // fallback label if the merge didn't carry through.
      const totalWeek = toNum(row[18]);
      if (totalWeek !== null) {
        const weekLabel = row[15] || `Week of ${row[0]}`;
