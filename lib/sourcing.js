...
export async function fetchDailyPurchasingSummary() {
  ...
  // Block 1 (rows 2+): Nabeel-LEGO cols 0-3, Hasan cols 5-8, sheet's own "Totals" (Nabeel+Hasan only) cols 10-13,
  // Weekly block cols 15-18.
  // Block 2, starting a few rows further down: Faqahat-ARRIS cols 0-3, Google Sourcing cols 5-8 — found dynamically
  // by locating the row containing "Faqahat's Sourcing" since its exact row number can shift as the sheet grows.
  ...
  // Block 2: Faqahat (ARRIS) + Google Sourcing, header row at block2HeaderIdx+1, data from +2
  ...
  // Merge all four sources into one combined daily total per date
  ...
  return { daily: daily.slice(-14), weekly, bySource: perSourceDaily };
}
