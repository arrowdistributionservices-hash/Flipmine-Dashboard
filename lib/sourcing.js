...
export async function fetchAllSourcingRecords() {
  const records = [];

  for (const sheet of AUTO_FEED_TABS) {
    let rows;
    try { rows = await fetchSheetCsv(sheet); } catch (e) { continue; }
    for (const r of rows) {
      const bought = (r['Bought (Y/N)'] || '').trim().toLowerCase();
      if (bought !== 'yes') continue;
